...
const faucet_usdc = async () => {
    ...
    try {
        ...
        const usdcMint = configData.usdcMint;
        ...
        const userToken = await getATAPublicKey(usdcMint, authority);

        const amount = 1000 * 1e9;
        // faucet usdc
        const tx = await program.rpc.faucetUsdc(new anchor.BN(amount), {
            accounts: {
                owner: authority,
                stateAccount,
                config,
