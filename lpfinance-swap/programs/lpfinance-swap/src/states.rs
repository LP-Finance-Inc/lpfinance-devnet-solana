...
impl PoolInfo {
    ...
    pub fn get_price(&self, tokena: Pubkey, tokenb: Pubkey) -> Result<u128> {
        if tokena == self.tokena_mint && tokenb == self.tokenb_mint {
            let price = Self::PRICE_MULTIPLIER * self.tokena_amount as u128 / self.tokenb_amount as u128;
            Ok( price )
        } else if tokena == self.tokenb_mint && tokenb == self.tokena_mint {
            let price = Self::PRICE_MULTIPLIER * self.tokenb_amount as u128 / self.tokena_amount as u128;
            Ok( price )
        } else {
            Ok(0)
        }
