...
const stable_swap = async () => {
  ...
  const user_ata_wsol = await findAssociatedTokenAddress(
    authKeypair.publicKey,
    WSOL
  );
  console.log('user_ata_wsol:', user_ata_wsol.toBase58());
  ...
  const amount_wsol = 10 * 1e9;

  const result = await program.rpc.swapNormalToLpusd(
    new anchor.BN(amount_wsol),
    {
        accounts: {
